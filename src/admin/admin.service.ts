...
export class AdminService {
  ...
  /**
   * Send welcome email with password and verification OTP
   */
      ...
      <html>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e5e7eb;">
            ...
            <p>Before you can access your account, please verify your email address using the OTP code below:</p>
            <div style="text-align: center; margin: 30px 0;">
              <div style="background: #ffffff; border: 2px solid #0284c7; border-radius: 8px; padding: 20px; display: inline-block;">
  ...
  async createUser(
    ...
    // Send welcome email with password and verification OTP
    await this.sendWelcomeEmail(email, firstName, generatedPassword, otpCode);
  ...
  async updateClub(id: string, data: any, userId?: string) {
    ...
    if (userId) {
      ...
      const isAdmin = user && ['SUPER_ADMIN', 'PLATFORM_ADMIN', 'ADMIN', 'COMMUNITY_MANAGER'].includes(user.role);

      // Check if user is a club manager (managers array is filtered to only include this user if they're a manager)
      const isManager = club.managers && Array.isArray(club.managers) && club.managers.length > 0;

      // Check if user is the creator
      const isCreator = club.createdBy === userId;

      if (!isAdmin && !isManager && !isCreator) {
        throw new ForbiddenException('Only admins, club managers, or club creators can update clubs');
      }
    }
  ...
  // Event Management
  async getAllEvents(userId: string, userRole: UserRole, page = 1, limit = 20) {
    const skip = (page - 1) * limit;
    
    // For club managers, only show events for their clubs
    let whereClause: any = {};
    if (userRole === UserRole.CLUB_MANAGER) {
      // Get all clubs where the user is a manager or lead
